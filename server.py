...
haURL = "http://homeassistant.local:8123/services/script"
haHeaders = {
    "Authorization": "",
...
def switchOn():
    ...
    blinkThread = threading.Thread(
        target=setColor, args=(rgb[0], rgb[1], rgb[2]))
    ...
    while getattr(crntT, "do_run", True):
        ...
        for x in range(0, width):
            for y in range(0, height):
                ...
                xy = x + y / 4
                r = (math.cos((x + i) / 2.0) +
                     math.cos((y + i) / 2.0)) * 64.0 + 128.0
                g = (math.sin((x + i) / 1.5) +
                     math.sin((y + i) / 2.0)) * 64.0 + 128.0
                b = (math.sin((x + i) / 2.0) +
                     math.cos((y + i) / 1.5)) * 64.0 + 128.0
...
def availableCall():
    with app.app_context():
        ...
        response = requests.post(f'{haURL}/office_light_available', headers = haHeaders)
...
def busyCall():
    with app.app_context():
        ...
        response = requests.post(f'{haURL}/office_light_busy', headers = haHeaders)
...
def awayCall():
    with app.app_context():
        ...
        response = requests.post(f'{haURL}/office_light_away', headers = haHeaders)
...
def apiDisplayRainbow():
    ...
    speed = content.get('speed', None)
    blinkThread = threading.Thread(
        target=displayRainbow, args=(brightness, speed, None))
    blinkThread.do_run = True
    blinkThread.start()
    setTimestamp()
    return make_response(jsonify())
...
def apiRotate():
    ...
    if globalStatus == 'Busy':
        awayCall()
    elif globalStatus == 'Away':
        availableCall()
    elif globalStatus == 'Available':
        apiOff()
    else:
        busyCall()

    return make_response(jsonify({'status': globalStatus}), 200)


@app.route('/api/togglemute', methods=['GET'])
def toggleMute():
    global globalStatus
    if globalStatus == 'Busy':
        awayCall()
...
@app.route('/api/statusraw', methods=['GET'])
def apiStatusRaw():
    global globalStatus
    return make_response(globalStatus, 200)
...
try:
    ...
    with app.app_context():
        button_a.when_pressed = apiOff
        button_b.when_pressed = awayCall
        button_x.when_pressed = availableCall
        button_y.when_pressed = busyCall
